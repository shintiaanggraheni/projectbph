...
if (isset($_POST["submitp"]))
{
	...
	echo "<script> 
		document.location.href = 'printba1.php?id=$id&stugas=$stgs'; 
		 </script>";	
}
...
<div class="form-style-3">
	<form action="" method="post">
		<fieldset><legend>Print Option </legend>
		<ul>	
<!-- 
				<input type="text" class="input-field" name="noorder" id="noorder" 
				value="<?= $id;  ?>" readonly>  -->

			<label for = "stugas" > No. Surat Tugas :  </label>
			<select name="stugas">
			<?php 		 
$cari=mysqli_query($konek,"SELECT stugas FROM tims WHERE idk = $id");
 while ($data=mysqli_fetch_array($cari)) { ?>	   
	<option value="<?=$data['stugas']?>"><?=$data['stugas']?></option> 
		<?php } ?>
			</select>

			<br><br>
				<button type ="submit" name="submitp">Print</button> 
