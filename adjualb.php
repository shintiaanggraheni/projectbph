...
$idjns		=htmlspecialchars($_POST["idjns"]);
$bulan 		=htmlspecialchars($_POST["bulan"]);
$vol 	  	=htmlspecialchars($_POST["vol"]);
$ket 	  	=htmlspecialchars($_POST["ket"]);

$sql = "INSERT INTO `dtjual2` (`idbu`,`idjns`, `bulan`, `vol`, `ket`)
VALUES ('$idbu','$idjns', '$bulan', '$vol', '$ket')";
...
if (mysqli_affected_rows($konek) > 0) {
	...
	// if ( tambah($_POST) > 0 ) {
			echo "<script>
					document.location.href = 'djual.php?id=$idbu';		 	
				</script>";
				// alert('sukses nambah');
...
<html>
<head>
	<title>Data Penjualan /komoditi/bulan</title>
...
<body oncontextmenu="return false;">
	...
	<form action="" method="post">
		<fieldset><legend>Data Produk Jual</legend>
		<ul>			
			<!-- <label for = "id" > No. ID BU : </label>  -->
		<input type="text" class="input-field" name="idbu" id="idbu" value="<?=$idbu?>" readonly hidden>
		<input type="text" class="input-field" name="idjns" id="idjns" value="<?=$idjns?>" readonly hidden>
		...
		<label for = "vol" >Volume (KL) :</label>
		<input type="text" class="input-field" name="vol" id="vol"> 
		
		<label for = "ket" >Keterangan:</label>
		<input type="text" class="input-field" name="ket" id="kett"> 
