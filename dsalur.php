...
if (isset($_POST["tampil"]))
  {
      ...
      $laporsql = query("SELECT * FROM laporan WHERE idk = $id AND thn LIKE '%$thc%' AND priod LIKE '%$twc%' ");
      // $laporsql = query("SELECT * FROM laporan WHERE idk = $id");
  }
...
<label style="color: blue; ">Triwulan :</label>
      <select name="twcari" style="cursor:pointer;" >
            <option value="">...</option>
  ...
  <td style="position: right">
        <button type ="submitt" name="tampil" style="width: 130px;height: 35px;font-size: 12px;cursor: pointer; " >Cari</button>   
  </td>
...
<!--     <td> <?= $rowb["tsalurthsbl"]; ?> </td>
    ...
    <td> <?php $hari11 = round((($rowb["dtstok"])/($rowb["dtsalur"])));
